...
void Sketching::random_int(){
    ...
    std::uniform_int_distribution<uint32_t> dis(0, pow(2,31) - 1);  // Range is [0, 2^31 - 1]
    uint32_t rand_int1 = dis(gen);  // Generate a random 32-bit integer in the range [0, 2^31 - 1]
    hash_a = rand_int1;
}
