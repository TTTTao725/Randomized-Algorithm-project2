 ...
 * @LastEditors: Tao
 * @LastEditTime: 2023-04-12 18:58:02
 * @Description: 
...
//* initialise the table size as the same to the number of updates
HashTable::HashTable(int w): TABLE_SIZE(pow(2, w)), table(new vector<key_value_pair>[pow(2, w)]) {
...
void HashTable::update(key_value_pair pair, int w) {
    int index = hash(pair.key, w);
...
int HashTable::search(unsigned int key, int w) {
    int index = hash(key, w);
    // cout << index << endl;
