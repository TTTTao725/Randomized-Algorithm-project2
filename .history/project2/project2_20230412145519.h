 ...
 * @LastEditors: Tao
 * @LastEditTime: 2023-04-12 14:55:19
 * @Description: 
...
public:
    ...
    unsigned int hash_h_modulo(unsigned int x);
...
unsigned int Sketching::hash_h_modulo(unsigned int x){
    //* mersenne prime
...
void Sketching::update(key_value_pair pair){
    sketch_list[hash_h(pair.key)] += hash_g(pair.key) * pair.value;
}
