 ...
 * @LastEditors: Tao
 * @LastEditTime: 2023-04-13 19:48:53
 * @Description: 
...
HashTable::HashTable(int w): TABLE_SIZE(pow(2, w-1)), table(new vector<key_value_pair>[int(pow(2, w-1))]) {
...
void HashTable::initialise(int w){
    ...
    table = new vector<key_value_pair> []();
    for (int i = 0; i < TABLE_SIZE; i++) {
        table[i] = vector<key_value_pair>();
    }
